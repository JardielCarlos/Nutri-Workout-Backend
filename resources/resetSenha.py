...
class ResetSenha(Resource):
  def post(self):
    ...
    msg = MailMessage("Redefinir senha", sender="serviceemail927@gmail.com", recipients=[usuario.email])
    ...
    reset_url = url_for('newsenha', token=token, _external=True)
    ...
    logger.error("Solicitacao feita com sucesso")
    codigo = Message(0, "Solicitação feita com sucesso")
    return marshal(codigo, msgFields), 200

    # except:
    #   logger.error("Erro ao recuperar senha")
    #   codigo = Message(2, "Error ao recuperar senha")
    #   return marshal(codigo, msgFields), 400
